...
# Buttons panel for inline help
def notes_panel():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("💾 Save Example", callback_data="notes:example")],
...
async def notes_cb(client: Client, query: CallbackQuery):
    ...
    if data == "example":
        await query.message.edit_text(
            "📌 To save a note:\nReply to a message and use `/save keyword`\nOr `/save keyword content`.",
            reply_markup=notes_panel(),
            parse_mode="markdown"
        )
    elif data == "format":
        await query.message.edit_text(
            "**📎 Formatting Guide**\n- Markdown supported\n- Buttons via `[text](url)` allowed.",
            reply_markup=notes_panel(),
            parse_mode="markdown"
        )
    await query.answer()

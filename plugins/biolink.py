...
LINK_RE = re.compile('(?:https?://\\S+|tg://\\S+|t\\.me/\\S+|telegram\\.me/\\S+|(?:[A-Za-z0-9-]+\\.)+[A-Za-z]{2,})', re.IGNORECASE)
...
def build_warning(count: int, user, reason: str, is_final: bool=False):
    ...
    msg = f'🔇 <b>Final Warning for {name}</b>\n\n{reason}\nYou have been <b>muted</b>.' if is_final else f"⚠️ <b>Warning {count}/3 for {name}</b>\n\n{reason}\nFix this before you're muted."
    return (msg, None)
...
async def bio_link_violation(client: Client, message: Message, user, chat_id: int) -> bool:
    ...
    if now - last < BIO_VIOLATION_TTL:
        logger.debug('Bio violation throttled for %s/%s', chat_id, user.id)
        return False
    ...
    if contains_link(bio):
        ...
        await handle_violation(client, message, user, chat_id, 'Your bio contains a link, which is not allowed.')
